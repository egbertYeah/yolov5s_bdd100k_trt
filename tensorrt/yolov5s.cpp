...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../yolov5s-bdd.wts");
...
int main(int argc, char** argv) {
    ...
    // ------------------------------------------------------------------------ // 
    bool INFERENCE_VIDEO = false;
    if (std::string(argv[2]).find("mp4") != std::string::npos)
    	INFERENCE_VIDEO = true;         // video input
    if(!INFERENCE_VIDEO)
    {
	    bool save_txt = false;  // save detection result into txt files
	    bool save_img = true;  // whether save the image results

	    std::vector<std::string> file_names;  // filename in folder
	    ...
	    for (int f = 0; f < (int)file_names.size(); f++)
	    {
	        ...
	        for (int b = 0; b < fcount; b++)
	        {
	            ...
	            if(save_txt)
	            {
	            	...
	            	std::ofstream destFile("../detection-results/" + txt_file, std::ios::out);
	            	if(!destFile)
	            	{
	            ...
	            if(save_img)
	            {
		            ...
		            cv::imwrite("../outputs/" + file_names[f - fcount + 1 + b], img);
